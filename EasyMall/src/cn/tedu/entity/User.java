...
|  1 | admin     | 123      | 炒鸡管理员 | admin@tedu.cn           |
	...
	public void check() throws MsgException{
		...
		throw new MsgException("请输入用户名！user");
		}
		if(WebUtils.check(password)){
		throw new MsgException("密码不能为空!user");
		}
		if(WebUtils.check(password2)){
		throw new MsgException("确认密码不能为空！user");
		}
		if(!password.equals(password2)){
		throw new MsgException("两次密码不相同！user");
		}
		if(WebUtils.check(nickname)){
		throw new MsgException("昵称不能为空！user");
		}
		if(WebUtils.check(email)){
		throw new MsgException("邮箱不能为空!user");
		}
		if(!email.matches("^\\w+@\\w+(\\.\\w+)+$")){
		throw new MsgException("邮箱格式不正确！user");
		}
	}
